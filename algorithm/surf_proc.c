/*
	...
	1995-2014 Daniel Garcia-Castellanos
	Check copyright and other information in tisc/doc/
*/
/*

#include "types_n_defs.h"
#define	NDERS   		8	/*Number of derivatives or surrounding nodes at every node*/
// D8 algorithm
...
extern int
	...
	pbc, 
	ifan, 
	zequi_1, zequi_2, zequi_3, zequi_4,
	z1_x1, z1_x2, z1_x3, z1_x4,
	z1_y1, z1_y2, z1_y3, z1_y4,
	z2_x1, z2_x2, z2_x3, z2_x4,
	z2_y1, z2_y2, z2_y3, z2_y4,
	z3_x1, z3_x2, z3_x3, z3_x4,
	z3_y1, z3_y2, z3_y3, z3_y4,
	z4_x1, z4_x2, z4_x3, z4_x4,
	z4_y1, z4_y2, z4_y3, z4_y4,
	**lake_former_step;
...
extern float
	...
	teq_ini, 
	teq_final, 
	zr1, zr2, zr3, zr4, z1_init, z1_final, z2_init, z2_final, z3_init, z3_final, z4_init, z4_final,
	nfifty; 
...
int     slopestats(int row, int col);
int     qstats(int row, int col);
int		write_topo(int icount1);
...
int     find_up_inlet (int row, int col, BOOL **done); 
...
int 	uplift (int row, int col, float dt_eros); 
int     zonal_uplift (int x1, int y1, int x2, int y2, int x3, int y3, int x4, int y4, float upr, float dt); 
int     const_topo (int x1, int y1, int x2, int y2, int x3, int y3, int x4, int y4, float height); 
int 	const_uplift (float dt, float Time, float dt_eros); 
...
int Surface_Transport (float **topo, float **topo_ant, float dt, float dt_eros, int erosed_model, int lake_instant_fill)
{
	...
	*/
	...
	if (ifan == 1){	// East boundary
		for (int i=0; i<Ny; i++){
			topo[i][Nx-1] = 0;
			topo_ant[i][Nx-1] = 0;
		}						
	}
	if (ifan == 2){	// South boundary
		for (int i=0; i<Nx; i++){
			topo[Ny-1][i] = 0;
		}						
	}

	if (ifan == 99){	// South boundary
		for (int i=0; i<Nx; i++){
			topo[Ny-1][i] = -200;
		}						
	}
	...
	FILE 	*file;
	file = fopen("apex.apx", "wt");
	fprintf(file, "#TISC output apex frequency\n");	
	for (int i=0; i<Ny; i++) for (int j=0; j<Nx; j++) {
		drainage[i][j].fanfreq = 0;
	}
	...
	for (int iter=0; iter<n_iters; iter++) {
		...
		//PRINT_ERROR("resort matrix ENDS");
	    Define_Drainage_Net(sortcell);
		//PRINT_ERROR("define drainage net ENDS");
		...
		//PRINT_ERROR("read file node defs ENDS");
	    Calculate_Discharge(sortcell, &total_lost_water, &total_evap_water);
		//PRINT_ERROR("calculate discharge ENDS");
												
	    Fluvial_Transport(sortcell, dt_st, erosed_model, &total_lost_sed_mass, lake_instant_fill);
					
		// char command2[300];
		// sprintf(command2, "cp apex.apx apex%.4f.apx", Time/Matosec);
		// system(command2);
		//PRINT_ERROR("FLUVIAL TRANSPORT ENDS");
	...
	for (int i=0; i<Ny; i++) {
		for (int j=0; j<Nx; j++) {
			fprintf(file, "%7.2f\t%7.2f\t%4d\n", (xmin+j*dx)/1000, (ymax-i*dy)/1000, drainage[i][j].fanfreq);
		}
	}	
	...
	if (verbose_level>=1) {
		...
		PRINT_SUMLINE("rain_now : %+8.2e m3/s  evap_wat: %+8.2e m3/s outp_water: %+8.2e m3/s", total_rain, total_evap_water, total_lost_water);
	...
	PRINT_SUMLINE("eros_fw: %+8.2e N",  fwerosmass*g);
	PRINT_SUMLINE("eros_bedrock: %+8.2e N",  bederosmass*g);
	{
		float error;
...
int Calculate_Discharge (struct GRIDNODE *sortcell, float *total_lost_water, float *total_evap_water) 
{
	/*
	  ...
	  Modifies the drainage network accounting for LAKE EVAPORATION and its reduction of lake area in case of ENDORHEISM.
	*/
	int 	il, row, col, drow, dcol;
	float	runoff, dd, dxdivdy=dx/dy;
	PRINT_DEBUG("Calculating discharge");
	/*
	  This bucle starts from the highest cell and
	...
	*/
	for (int isort=0; isort < Nx*Ny; isort++) {
    		...
    		/*Check if this is an endorheic lake node and then check if the lake surface must be lowered by making this a river node*/
    		if (il) {
    		    float lake_evap=0;
    		...
    		/*Transfers water.*/
    		if (IN_DOMAIN(drow, dcol)) {
               	/*m berry addition of treating loss rate of a river correlated to it's width, which is connected to discharge. */
				if (diff_loss == 1) {
						float  Kw=1.1, aw=0.5, kr = 16, ar = 0.9;
						double Q_loss = kr * pow(Kw, ar) * pow(drainage[row][col].discharge, aw * ar) * dd * retct;
						*total_evap_water		     += Q_loss;
						drainage[row][col].discharge     -= Q_loss;
						drainage[row][col].loss = Q_loss; 
						drainage[row][col].width = Kw * pow(drainage[row][col].discharge, aw); 
				}
				/*M Berry end */

				if (diff_loss == 2) {
					float  kr = 16, ar = 0.9; // 10 mm median grain size is assumed; subject to change
					double eqn_1 = (4.63 / pow(g, 0.2)) * pow(drainage[row][col].discharge, 0.4);
					double eqn_2 = pow(drainage[row][col].discharge, 0.0667) / pow(sqrt(g*nfifty)*nfifty*nfifty, 0.0667);
					double Q_loss = kr * pow(eqn_1 * eqn_2, ar) * dd * retct; 
					*total_evap_water += Q_loss;
					drainage[row][col].discharge -= Q_loss;
					drainage[row][col].loss = Q_loss; 
					drainage[row][col].width = eqn_1 * eqn_2; 
				}
    			...
    			if (AT_BORDER(row,col)) { // when if a node is NOT in the domain, returns water back into the domain
	...
	for (int il=1; il<=nlakes; il++) {
		...
		Lake[il].vol = 0;
		for (int i=0; i<Lake[il].n; i++)  {
			Lake[il].vol +=  (Lake[il].alt - topo[Lake[il].row[i]][Lake[il].col[i]]);
		}
		Lake[il].vol *= dx*dy;
	}
	...
	/*CHECKS:*/
	for (int il=1; il<=nlakes; il++) {
	    float diff, lake_evap=0, max_elev, max_evap=0;
	    for (int i=0; i<Lake[il].n; i++) {
			lake_evap += evaporation[Lake[il].row[i]][Lake[il].col[i]]; max_evap=MAX_2(max_evap,evaporation[Lake[il].row[i]][Lake[il].col[i]]);
		}
	    lake_evap *= dx*dy;
	    ...
	    if (Lake[il].n_sd) {
			if (diff=(lake_evap - Lake_Input_Discharge(il))<0) {
				...
				if (!its_sea){
					if (diff>total_rain/200) {
						PRINT_WARNING("Lake %d (open; not sea; %d nodes) has less input %.2f m3/s than evap. %.2f m3/s.", 
				        il, Lake[il].n, Lake_Input_Discharge(il), lake_evap);
					}
				}				
			}
	    }
	    ...
	    if (Lake[il].alt != topo[Lake[il].row[Lake[il].n-1]][Lake[il].col[Lake[il].n-1]]) {
	    	...
	    	IF_LAKE_IS_SEA(il) its_sea=1;
	    	if (!its_sea) {
	    	    PRINT_ERROR("\aLake %d (not sea) should have the altitude of lake's last node %.2f m instead of %.2f m.", 
		        il, topo[Lake[il].row[Lake[il].n-1]][Lake[il].col[Lake[il].n-1]], Lake[il].alt);
				for (int i=0; i<Lake[il].n; i++) {
					PRINT_WARNING("[%d][%d] elevation: %.2f", Lake[il].row[i], Lake[il].col[i], topo[Lake[il].row[i]][Lake[il].col[i]]);
				}
	        }
	    }
	    /*Check: the lake's registered altitude should be the maximum among lake nodes (except for the sea)*/
			//commenting this out because it bothers me.
	    /*    for (int i=0,max_elev=-1e9; i<Lake[il].n; i++) max_elev = MAX_2(topo[Lake[il].row[i]][Lake[il].col[i]], max_elev);
	    if (Lake[il].alt != max_elev) {
	    	BOOL its_sea=0;
	    	IF_LAKE_IS_SEA(il) its_sea=1;
	    	if (!its_sea)
	    	    PRINT_ERROR("\aLake %d (not sea) should have the elevation of its highest node %.2f m instead of %.2f m.", il, max_elev, Lake[il].alt);
	    }*/
...
int const_uplift (float dt, float Time, float dt_eros){
	if (!dequim) return (0);
	for (int i=0; i<Ny; i++) for (int j=0; j<Nx; j++) {
		   // constant uplift rate (spatially uniform)
	    uplift(i, j, dt);
	}
}
...
int Define_Drainage_Net (struct GRIDNODE *sortcell) 
{
	...
	int ro1[NDERS], co1[NDERS];
	int ro2[NDERS], co2[NDERS];
	int ro3[NDERS-3], co3[NDERS-3];
	int ro4[NDERS-3], co4[NDERS-3];
	int ro5[NDERS-3], co5[NDERS-3];
	int ro6[NDERS-3], co6[NDERS-3];
	...
	/*Create lake #0 which will contain nothing*/
	...
	for (int isort=Nx*Ny-1; isort>=0; isort--) {
		...
		if (pbc == 1){ // lateral boundaries
			// D8 definition on the Western edge (not on vertices)
			ro1[0]=i-1, ro1[1]=i,   ro1[2]=i+1, ro1[3]=i,    ro1[4]=i-1, ro1[5]=i+1, ro1[6]=i+1,  ro1[7]=i-1;
			co1[0]=j,   co1[1]=j+1, co1[2]=j,   co1[3]=Nx-1, co1[4]=j+1, co1[5]=j+1, co1[6]=Nx-1, co1[7]=Nx-1;
			// D8 definition on the Eastern edge (not on vertices)
			ro2[0]=i-1, ro2[1]=i, ro2[2]=i+1, ro2[3]=i,   ro2[4]=i-1, ro2[5]=i+1, ro2[6]=i+1, ro2[7]=i-1;
			co2[0]=j,   co2[1]=0, co2[2]=j,   co2[3]=j-1, co2[4]=0,   co2[5]=0,   co2[6]=j-1, co2[7]=j-1;
			// D8 definition on the corners
			// NW (0, 0)
			ro3[0]=i,    ro3[1]=i,   ro3[2]=i+1,  ro3[3]=i+1, ro3[4]=i+1;
			co3[0]=Nx-1, co3[1]=j+1, co3[2]=Nx-1, co3[3]=j,   co3[4]=j+1;
			// SW (Ny-1, 0)
			ro4[0]=i,    ro4[1]=i,   ro4[2]=i-1,  ro4[3]=i-1, ro4[4]=i-1;
			co4[0]=Nx-1, co4[1]=j+1, co4[2]=Nx-1, co4[3]=j,   co4[4]=j+1;
			// NE (0, Nx-1)
			ro5[0]=i,    ro5[1]=i,   ro5[2]=i+1,  ro5[3]=i+1, ro5[4]=i+1;
			co5[0]=j-1,  co5[1]=0,   co5[2]=j-1,  co5[3]=j,   co5[4]=0;
			// SE (Ny-1, Nx-1)
			ro6[0]=i,    ro6[1]=i,   ro6[2]=i-1,  ro6[3]=i-1, ro6[4]=i-1;
			co6[0]=j-1,  co6[1]=0,   co6[2]=j-1,  co6[3]=j,   co6[4]=0;
		}		
		...
		/*Calculate derivatives and look for undefined lakes in all directions, again for the periodic boundaries */
		if (pbc == 1){
			if (j == 0 && i != 0 && i != Ny-1){
				for (int l=undef_lake_adj=0; l<NDERS; l++) { 
					float dist, deriv;
					...
					if (IN_DOMAIN(ro1[l],co1[l])) {
						deriv = (topo[ro1[l]][co1[l]]-topo[i][j])/dist;
						if (drainage[ro1[l]][co1[l]].lake < 0){								
							undef_lake_adj = drainage[ro1[l]][co1[l]].lake;
						}
						else{
							if (deriv<0 && deriv<maxderneg_noundef) { //030921
								imaxderneg_noundef=l; maxderneg_noundef=deriv;
							}
						}
					}
					else {
					...
					if (deriv != SIGNAL) {
						...
						if (deriv<=0) n_nonposders++;
						if (deriv==0) n_zeroders++;
						if (deriv<0 && drainage[ro1[l]][co1[l]].lake >= 0) n_negders_nonundef_lake++;
						if (deriv<0 && deriv<maxderneg) {imaxderneg=l; maxderneg=deriv;}
					}						
				}	
			}
				...
				for (int l=undef_lake_adj=0; l<NDERS; l++) { 
					...
					if (IN_DOMAIN(ro2[l],co2[l])) {
						deriv = (topo[ro2[l]][co2[l]]-topo[i][j])/dist;
						if (drainage[ro2[l]][co2[l]].lake < 0){								
							undef_lake_adj = drainage[ro2[l]][co2[l]].lake;
						}
						else{
							if (deriv<0 && deriv<maxderneg_noundef) { //030921
								imaxderneg_noundef=l; maxderneg_noundef=deriv;
					...
					if (deriv != SIGNAL) {
						...
						if (deriv<=0) n_nonposders++;
						if (deriv==0) n_zeroders++;
						if (deriv<0 && drainage[ro2[l]][co2[l]].lake >= 0) n_negders_nonundef_lake++;
						if (deriv<0 && deriv<maxderneg) {imaxderneg=l; maxderneg=deriv;}
					}						
				}	
			}
		...
		/*Normal (non-sea) lakes:*/
			...
			if (undef_lake_adj) {
				...
				if (pbc == 1){
					if (j == 0 && i != 0 && i != Ny-1){
						if (n_negders_nonundef_lake) {
							if (IN_DOMAIN(ro1[imaxderneg_noundef],co1[imaxderneg_noundef])) {
								Add_Node_To_Lake(i,j, undef_lake_adj);
								Add_Outlet_To_Lake(i,j,ro1[imaxderneg_noundef],co1[imaxderneg_noundef], undef_lake_adj);
								switch_outlet=YES;
							}
						}
						else {
						...
						if (n_negders_nonundef_lake) {
							if (IN_DOMAIN(ro2[imaxderneg_noundef],co2[imaxderneg_noundef])) {
								Add_Node_To_Lake(i,j, undef_lake_adj);
								Add_Outlet_To_Lake(i,j,ro2[imaxderneg_noundef],co2[imaxderneg_noundef], undef_lake_adj);
								switch_outlet=YES;
							}
						}
						else {
				...
				if (n_zeroders) {
					if (n_negders) {
						...
						if (pbc == 1){
							if (j == 0 && i != Ny-1 && i != 0){
								Add_Node_To_Lake(i,j, i_lake=New_Lake());
								Add_Outlet_To_Lake(i,j, ro1[imaxderneg], co1[imaxderneg], i_lake);
							}
							else if (j == Nx-1 && i != Ny-1 && i != 0){
								Add_Node_To_Lake(i,j, i_lake=New_Lake());
								Add_Outlet_To_Lake(i,j, ro2[imaxderneg], co2[imaxderneg], i_lake);
							}
							else{
								Add_Node_To_Lake(i,j, i_lake=New_Lake());
								Add_Outlet_To_Lake(i,j, ro[imaxderneg], co[imaxderneg], i_lake);
		...
		if (drainage[i][j].lake<0 && undef_lake_adj) {
			...
			if (pbc == 1){
				if (j == 0 && i != Ny-1 && i != 0){
					for (int l=0; l<NDERS; l++) {
						if (IN_DOMAIN(ro1[l],co1[l])) {
							int il = drainage[ro1[l]][co1[l]].lake;
							if (il<0 && il != drainage[i][j].lake) {
								Unify_Lakes(drainage[i][j].lake, il);
							}
						}
					...
					for (int l=0; l<NDERS; l++) {
						if (IN_DOMAIN(ro2[l],co2[l])) {
							int il = drainage[ro2[l]][co2[l]].lake;
							if (il<0 && il != drainage[i][j].lake) {
								Unify_Lakes(drainage[i][j].lake, il);
							}
						}
		...
		if (pbc == 1){ 
			if (drainage[i][j].lake && i == 0 && !switch_outlet) {
				if (eros_bound_cond[BORDER_INDEX(i,j)] != 'c'){
					Add_Outlet_To_Lake(i,j,SIGNAL,SIGNAL,drainage[i][j].lake);
				}
			}
			if (drainage[i][j].lake && i == Ny-1 && !switch_outlet) {
				if (eros_bound_cond[BORDER_INDEX(i,j)] != 'c'){
					Add_Outlet_To_Lake(i,j,SIGNAL,SIGNAL,drainage[i][j].lake);
				}
			}
		...
		if (topo[i][j]>sea_level) {
			...
			*/
			...
			/*
			  ...
			  last (uppermost) node, then define all 'sea-like' lakes.
			*/
			if (isort>0) {
		...
		*/
		...
		if (pbc == 1){
			if (!drainage[i][j].lake) {
				drainage[i][j].type = 'R';
				if (imaxderneg != SIGNAL) {
			    	/*Drain to the lowest neighbor*/
					if (j == 0 && i != Ny-1 && i != 0){
						drainage[i][j].dr_row = ro1[imaxderneg];
						drainage[i][j].dr_col = co1[imaxderneg];
					}
					else if (j == Nx-1 && i != Ny-1 && i != 0){
						drainage[i][j].dr_row = ro2[imaxderneg];
						drainage[i][j].dr_col = co2[imaxderneg];
					}
					else{
						drainage[i][j].dr_row = ro[imaxderneg];
						drainage[i][j].dr_col = co[imaxderneg];
	...
	*/
	for (int l=1; l<=nlakes; l++) {
		...
		for (int m=0; m<Lake[l].n; m++) {
			...
			for (int n=0,mindist2=1e24; n<Lake[l].n_sd; n++) {
	...
	for (int il=1,total_lake_nodes=0; il<=nlakes; il++) total_lake_nodes += Lake[il].n;
	{
		int k=0;
...
int Fluvial_Transport(struct GRIDNODE *sortcell, float dt_st, int erosed_model, float *total_lost_sed_mass, int lake_instant_fill){
	int 	row, col, drow, dcol, co[NDERS], ro[NDERS], co1[NDERS], ro1[NDERS], co2[NDERS], ro2[NDERS]; 
	int		co3[NDERS-3], ro3[NDERS-3], co4[NDERS-3], ro4[NDERS-3], co5[NDERS-3], ro5[NDERS-3], co6[NDERS-3], ro6[NDERS-3];
	float	d_mass; 	/*Increment of suspended mass in this cell (positive means erosion).*/
	float s1, s2, s3, s4, s5, s6, s7, s8;
	float s11, s12, s13, s14, s15, s16, s17, s18;
	float savg, sstdev, smax, smin;
	float savg1, sstdev1, smax1, smin1;
	int drtemp, dctemp = 0;
	int urow1=0, ucol1=0, urow2=0, ucol2=0, drow2=0, dcol2=0, urow3=0, ucol3=0;
	int urow; 
	int ucol;
	float uslope1, uslope2, dslope2, dslope3;
	float dist1, dist2, dist3;
	int ddcol, ddrow, dddcol, dddrow;
    ...
    int bp1;
    float streamtopo[Nx];
    float streamslope[Nx];
    float streamtopo_d[Nx];
    float streamslope_d[Nx];
	int urowtemp;
	int ucoltemp;
	int drowtemp;
	int dcoltemp;
	...
	*/
	if (erosed_model>=2) for (int isort=0; isort < Nx*Ny; isort++) {
		...
		if (pbc == 1){ // lateral boundaries
			// D8 definition on the Western edge (not on vertices)
			ro1[0]=row-1, ro1[1]=row,   ro1[2]=row+1, ro1[3]=row,    ro1[4]=row-1, ro1[5]=row+1, ro1[6]=row+1,  ro1[7]=row-1;
			co1[0]=col,   co1[1]=col+1, co1[2]=col,   co1[3]=Nx-1,   co1[4]=col+1, co1[5]=col+1, co1[6]=Nx-1,   co1[7]=Nx-1;
			// D8 definition on the Eastern edge (not on vertices)
			ro2[0]=row-1, ro2[1]=row, ro2[2]=row+1, ro2[3]=row,   ro2[4]=row-1, ro2[5]=row+1, ro2[6]=row+1, ro2[7]=row-1;
			co2[0]=col,   co2[1]=0,   co2[2]=col,   co2[3]=col-1, co2[4]=0,     co2[5]=0,     co2[6]=col-1, co2[7]=col-1;
			// NW (0, 0)
			ro3[0]=row,    ro3[1]=row,   ro3[2]=row+1,  ro3[3]=row+1, ro3[4]=row+1;
			co3[0]=Nx-1,   co3[1]=col+1, co3[2]=Nx-1,   co3[3]=col,   co3[4]=col+1;
			// SW (Ny-1, 0)
			ro4[0]=row,    ro4[1]=row,   ro4[2]=row-1,  ro4[3]=row-1, ro4[4]=row-1;
			co4[0]=Nx-1,   co4[1]=col+1, co4[2]=Nx-1,   co4[3]=col,   co4[4]=col+1;
			// NE (0, Nx-1)
			ro5[0]=row,    ro5[1]=row,   ro5[2]=row+1,  ro5[3]=row+1, ro5[4]=row+1;
			co5[0]=col-1,  co5[1]=0,     co5[2]=col-1,  co5[3]=col,   co5[4]=0;
			// SE (Ny-1, Nx-1)
			ro6[0]=row,    ro6[1]=row,   ro6[2]=row-1,  ro6[3]=row-1, ro6[4]=row-1;
			co6[0]=col-1,  co6[1]=0,     co6[2]=col-1,  co6[3]=col,   co6[4]=0;
		}

		/*Calculate max and min height in the 8 surrounding points:*/
		if (pbc == 0){
		...
		if (pbc == 1){
			maxsorr=minsorr=topo[row][col];
			for(int i=0; i<NDERS; i++) {
				if (col == 0 && row != 0 && row != Ny-1) {
					if (IN_DOMAIN(ro1[i],co1[i])) {
						minsorr = MIN_2(minsorr, topo[ro1[i]][co1[i]]);
						maxsorr = MAX_2(maxsorr, topo[ro1[i]][co1[i]]);
					}	    			
				}
				else if (col == Nx-1 && row != 0 && row != Ny-1) {
					if (IN_DOMAIN(ro2[i],co2[i])) {
						minsorr = MIN_2(minsorr, topo[ro2[i]][co2[i]]);
						maxsorr = MAX_2(maxsorr, topo[ro2[i]][co2[i]]);
					}
				}	
				if (row == 0 && col == 0) {
					if (i > NDERS-4){break;}
					if (IN_DOMAIN(ro3[i],co3[i])) {
						minsorr = MIN_2(minsorr, topo[ro3[i]][co3[i]]);
						maxsorr = MAX_2(maxsorr, topo[ro3[i]][co3[i]]);
					}
				}
				if (row == Ny-1 && col == 0) {
					if (i > NDERS-4){break;}
					if (IN_DOMAIN(ro4[i],co4[i])) {
						minsorr = MIN_2(minsorr, topo[ro4[i]][co4[i]]);
						maxsorr = MAX_2(maxsorr, topo[ro4[i]][co4[i]]);
					}
				}
				if (row == 0 && col == Nx-1) {
					if (i > NDERS-4){break;}
					if (IN_DOMAIN(ro5[i],co5[i])) {
						minsorr = MIN_2(minsorr, topo[ro5[i]][co5[i]]);
						maxsorr = MAX_2(maxsorr, topo[ro5[i]][co5[i]]);
					}
				}
				if (row == Ny-1 && col == Nx-1) {
					if (i > NDERS-4){break;}
					if (IN_DOMAIN(ro6[i],co6[i])) {
						minsorr = MIN_2(minsorr, topo[ro6[i]][co6[i]]);
						maxsorr = MAX_2(maxsorr, topo[ro6[i]][co6[i]]);
					}
				}			
				else{
					if (IN_DOMAIN(ro[i],co[i])) {
		...
		/* periodic addition ends */
	    /*
	      Finds the height of the minimum surrounding contributor
	    ...
	    */
		...
		if (pbc == 1){
	    	...
	    	for(int i=0; i<NDERS; i++) {
	    	    if (col == 0 && row != 0 && row != Ny-1) {
					if (IN_DOMAIN(ro1[i],co1[i])) {
						if (drainage[ro1[i]][co1[i]].dr_row == row && drainage[ro1[i]][co1[i]].dr_col == col) {
	    			    	if (drainage[ro1[i]][co1[i]].discharge>main_tribut_disch) {
	    				    	float dist;
	    				    	switch (i) {
	    					    	case 0: case 2: dist=dy;  break;   /*N,S*/
	    					    	case 1: case 3: dist=dx;  break;   /*E,W*/
	    				    	...
	    				    	main_tribut_disch = drainage[ro1[i]][co1[i]].discharge;
	    				    	main_tribut_slope = (topo[row][col]-topo[ro1[i]][co1[i]])/dist;
	    				    	main_tribut_alt   = topo[ro1[i]][co1[i]];
	    			    	}
	    			    	minsorr_trib = MIN_2(minsorr_trib, topo[ro1[i]][co1[i]]);
	    		    	}
					}
	    	    }
				else if (col == Nx-1 && row != 0 && row != Ny-1) {
					if (IN_DOMAIN(ro2[i],co2[i])) {
						if (drainage[ro2[i]][co2[i]].dr_row == row && drainage[ro2[i]][co2[i]].dr_col == col) {
	    			    	if (drainage[ro2[i]][co2[i]].discharge>main_tribut_disch) {
	    				    	float dist;
	    				    	switch (i) {
	    					    	case 0: case 2: dist=dy;  break;   /*N,S*/
	    					    	case 1: case 3: dist=dx;  break;   /*E,W*/
	    				    	...
	    				    	main_tribut_disch = drainage[ro2[i]][co2[i]].discharge;
	    				    	main_tribut_slope = (topo[row][col]-topo[ro2[i]][co2[i]])/dist;
	    				    	main_tribut_alt   = topo[ro2[i]][co2[i]];
	    			    	}
	    			    	minsorr_trib = MIN_2(minsorr_trib, topo[ro2[i]][co2[i]]);
	    		    	}
					}
	    	    }
				else if (row == 0 && col == 0) {
					if (i > NDERS-4){break;}
					if (IN_DOMAIN(ro3[i],co3[i])) {
						if (drainage[ro3[i]][co3[i]].dr_row == row && drainage[ro3[i]][co3[i]].dr_col == col) {
	    			    	if (drainage[ro3[i]][co3[i]].discharge>main_tribut_disch) {
	    				    	float dist;
	    				    	switch (i) {
	    					    	case 0: case 2: dist=dy;  break;   /*N,S*/
	    					    	case 1: case 3: dist=dx;  break;   /*E,W*/
	    				    	...
	    				    	main_tribut_disch = drainage[ro3[i]][co3[i]].discharge;
	    				    	main_tribut_slope = (topo[row][col]-topo[ro3[i]][co3[i]])/dist;
	    				    	main_tribut_alt   = topo[ro3[i]][co3[i]];
	    			    	}
	    			    	minsorr_trib = MIN_2(minsorr_trib, topo[ro3[i]][co3[i]]);
	    		    	}
					}
	    	    }
					...
					if (i > NDERS-4){break;}
					if (IN_DOMAIN(ro4[i],co4[i])) {
						if (drainage[ro4[i]][co4[i]].dr_row == row && drainage[ro4[i]][co4[i]].dr_col == col) {
	    			    	if (drainage[ro4[i]][co4[i]].discharge>main_tribut_disch) {
	    				    	float dist;
	    				    	switch (i) {
	    					    	case 0: case 2: dist=dy;  break;   /*N,S*/
	    					    	case 1: case 3: dist=dx;  break;   /*E,W*/
	    				    	...
	    				    	main_tribut_disch = drainage[ro4[i]][co4[i]].discharge;
	    				    	main_tribut_slope = (topo[row][col]-topo[ro4[i]][co4[i]])/dist;
	    				    	main_tribut_alt   = topo[ro4[i]][co4[i]];
	    			    	}
	    			    	minsorr_trib = MIN_2(minsorr_trib, topo[ro4[i]][co4[i]]);
	    		    	}
					}
	    	    }
					...
					if (i > NDERS-4){break;}
					if (IN_DOMAIN(ro5[i],co5[i])) {
						if (drainage[ro5[i]][co5[i]].dr_row == row && drainage[ro5[i]][co5[i]].dr_col == col) {
	    			    	if (drainage[ro5[i]][co5[i]].discharge>main_tribut_disch) {
	    				    	float dist;
	    				    	switch (i) {
	    					    	case 0: case 2: dist=dy;  break;   /*N,S*/
	    					    	case 1: case 3: dist=dx;  break;   /*E,W*/
	    				    	...
	    				    	main_tribut_disch = drainage[ro5[i]][co5[i]].discharge;
	    				    	main_tribut_slope = (topo[row][col]-topo[ro5[i]][co5[i]])/dist;
	    				    	main_tribut_alt   = topo[ro5[i]][co5[i]];
	    			    	}
	    			    	minsorr_trib = MIN_2(minsorr_trib, topo[ro5[i]][co5[i]]);
	    		    	}
					}
	    	    }
					...
					if (i > NDERS-4){break;}
					if (IN_DOMAIN(ro6[i],co6[i])) {
						if (drainage[ro6[i]][co6[i]].dr_row == row && drainage[ro6[i]][co6[i]].dr_col == col) {
	    			    	if (drainage[ro6[i]][co6[i]].discharge>main_tribut_disch) {
	    				    	float dist;
	    				    	switch (i) {
	    					    	case 0: case 2: dist=dy;  break;   /*N,S*/
	    					    	case 1: case 3: dist=dx;  break;   /*E,W*/
	    				    	...
	    				    	main_tribut_disch = drainage[ro6[i]][co6[i]].discharge;
	    				    	main_tribut_slope = (topo[row][col]-topo[ro6[i]][co6[i]])/dist;
	    				    	main_tribut_alt   = topo[ro6[i]][co6[i]];
	    			    	}
	    			    	minsorr_trib = MIN_2(minsorr_trib, topo[ro6[i]][co6[i]]);
	    		    	}
					}
	    	    }
				else{
	    ...
	    */
	    switch (drainage[row][col].type) {
	    	...
	    	case 'E':
				...
				//dist_array[isort]=dist /* save the distance calculated ~m berry 3/30/2017 */
		...
		switch (erosed_model) {
			...
			#define ERODED_ERODIBILITY   /*Takes a mean erodibility*/ float depth2average=10., dh, weight, totalweight=0, basedepth=0, erodibility_aux=0;\
				for (int i=numUnits-1; i>=0; i--) {\
					basedepth+=Units[i].thick[row][col];\
					basedepth=MIN_2(basedepth,depth2average+.1);\
					weight=Units[i].thick[row][col]/(basedepth+1.); totalweight+=weight;\
			...
			#define TRANSPORT_BOUNDARY_CONDITIONS \
				if (AT_BORDER(row,col)) {\
					switch (eros_bound_cond[BORDER_INDEX(row,col)]) {\
						...
						case '9':   transp_capacity_eq *= 1e6;	    break;\
					}\
				}	
		case 2:
		...
		case 3:
			...
			if (transp_capacity_eq >= drainage[row][col].masstr) {
				...
				d_mass = (transp_capacity_eq - drainage[row][col].masstr) * dt_st;
			}
			break;
		...
		case 4:
			...
			//* new case by Michael Berry and Dan Cadol */
			...
			case 8:
				...
				/*any additional variables here*/
					...
					/*scaling factor*/
					//double q_ratio = (drainage[row][col].masstr/transp_capacity_eq);
...
case 107: 
		...
		if (ifan == 2){ // zero elevation boundary
			if (row == Ny-1){
				d_mass = 0;
			}
		}

		if (ifan == 99){ // zero elevation boundary
			if (row == Ny-1){
				d_mass = 0;
			}
		}

	    /*Limit d_mass with the surrounding topo*/
	    switch (drainage[row][col].type) {
	      float Dhsed;
	      case 'L':
	    	    /*Sedimentation will be done in Lake_Fill().*/
	    ...
	    if (IN_DOMAIN(drow,dcol)) {
			...
			switch (drainage[drow][dcol].type) {
		    	case 'L':
					...
					if (Lake[ild].n_sd) {
						float diff;
						/*Check: this can happen when a node transferring to a lake is eroded below the lake level 
						or when the lake node was already deposited and became higher than the tributary node. See Lake_Fill at Dhsed=...*/
						if (IN_DOMAIN(drainage[drow][dcol].dr_row, drainage[drow][dcol].dr_col)) {
							if (diff=(topo[drainage[drow][dcol].dr_row][drainage[drow][dcol].dr_col] - topo[row][col]) > 0){
								if (fabs(diff)>2 || verbose_level>=3){
									PRINT_ERROR("[%d][%d] transferring mass to lake in [%d][%d] is < than outlet [%d][%d] by %.1f m.", 
									row, col, drow, dcol, drainage[drow][dcol].dr_row, drainage[drow][dcol].dr_col, diff);
								}
							}
						} 
						/*
					...
					/*Next line commented in tAo!!*/
					drainage[drow][dcol].masstr = 0; // downstream mass transport value in lake
				break;
				case 'R':
				case 'E':
				drainage[drow][dcol].masstr += drainage[row][col].masstr;
	...
	for (;;) {
	    ...
	    for (int m=0; m<n_done; m++) {
			...
			/*if it's in the lake but not treated*/
				...
				if (pbc == 1){					
					if (i != 0 && i != Ny-1 && j == 0){
						if (IN_DOMAIN(ro1[k],co1[k])) {
							if (drainage[ro1[k]][co1[k]].lake == il) {
								if (!done_grid[ro1[k]][co1[k]]) {
										to_do[n_to_do].row = ro1[k];
										to_do[n_to_do].col = co1[k];
										n_to_do++;
								}
								if (k<4) {total_weight_distr += 1;        done_grid[ro1[k]][co1[k]] += 1;}
								else     {total_weight_distr += 1/1.4142; done_grid[ro1[k]][co1[k]] += 1/1.4142;}
							}
						}						
					}
					else if (i != 0 && i != Ny-1 && j == Nx-1){
						if (IN_DOMAIN(ro2[k],co2[k])) {
							if (drainage[ro2[k]][co2[k]].lake == il) {
								if (!done_grid[ro2[k]][co2[k]]) {
										to_do[n_to_do].row = ro2[k];
										to_do[n_to_do].col = co2[k];
										n_to_do++;
								}
								if (k<4) {total_weight_distr += 1;        done_grid[ro2[k]][co2[k]] += 1;}
								else     {total_weight_distr += 1/1.4142; done_grid[ro2[k]][co2[k]] += 1/1.4142;}
							}
						}						
					}
					else{
...
int Erode (double d_mass, int row, int col)
{
	...
	*/
	...
	for (int k=numUnits-1; mass_per_m2>0 && k>=0; k--) {
		if (Units[k].density == denssedim) {
			...
			if (row < 41){
				fwerosmass += dh_eros_unit * Units[k].density * dx * dy;
			}
			if (Units[k].density == denscrust){
				if (row < 41){
					bederosmass += dh_eros_unit * Units[k].density * dx * dy;
				}				
			}
		}
	...
	if (mass_per_m2>0) {
		...
		if (row < 41){
			fwerosmass += mass_per_m2 * dx*dy;
			bederosmass += mass_per_m2 * dx*dy;
		}
	}
...
int zonal_uplift (int x1, int y1, int x2, int y2, int x3, int y3, int x4, int y4, float upr, float dt){
	// x1 = x3; x2 = x4; y1 = y2; y3 = y4; for rectangular zones, no interpolation
	// coordinate sorting order = NW - NE - SE - SW
	float dh_uplift = 0;
	dh_uplift = (upr/1000/60/60/24/365)*dt;
	for (int i=x1; i<=x2; i++){
		for (int j=y1; j<=y3; j++){
			//PRINT_ERROR("uprate %1.5f", upr);
			//PRINT_ERROR("ZONAL UPLIFT APPLIED %1.5f", dh_uplift);
			//topo[j][i] += dh_uplift;
			Units_base[j][i] += dh_uplift;
			//Units[0].thick[j][i] += dh_uplift;
		}
	}
	return (1);
}

/* maintain constant height over the designated area*/
int const_topo (int x1, int y1, int x2, int y2, int x3, int y3, int x4, int y4, float height){
	// x1 = x3; x2 = x4; y1 = y2; y3 = y4; for rectangular zones, no interpolation
	// coordinate sorting order = NW - NE - SE - SW
	for (int i=x1; i<=x2; i++){
		for (int j=y1; j<=y3; j++){
			//PRINT_ERROR("constant topographic height %1.5f", height);
			topo[j][i] = height;
			Units_base[j][i] = height;
		}
	}
	return (1);
}
...
/*
THIS SUBROUTINE APPLIES UNIFORM UPLIFT ALL OVER THE MODEL DOMAIN WHEN
"dequim" IN PRM IS SET TO BE 1. THE PRESCRIBED UPLIFT RATE OF eqr IS USED
DURING A PERIOD FROM teq_ini TO teq_final.
*/
int uplift (int row, int col, float dt){
	float dh_eros = 0;
	//float d_mass = 0;
	dh_eros = (eqr/1000/60/60/24/365)*dt; // m/s times dt in seconds
	/* No uplift along boundaries */
	Dq[row][col] +=  dh_eros * g * (denssedim-densenv);
	topo[row][col] += dh_eros;
	Units_base[row][col] += dh_eros; 
	return (1);
}
...
{   
	...
	*/
	int 	i, j, k, il, n_neigh_same_lake=0, splitting=0,
		    ro[NDERS], co[NDERS], ro1[NDERS], co1[NDERS], ro2[NDERS], co2[NDERS];
	int     ro3[NDERS-3], co3[NDERS-3], ro4[NDERS-3], co4[NDERS-3], ro5[NDERS-3], co5[NDERS-3], ro6[NDERS-3], co6[NDERS-3];
	il = drainage[row][col].lake;
	...
	if (pbc == 1){		
		if (row != 0 && row != Ny-1 && col == 0){
			for (k=0; k<NDERS; k++) {
				if (IN_DOMAIN(ro1[k],co1[k])) {
					if (drainage[ro1[k]][co1[k]].lake == il) n_neigh_same_lake++;
				}
			}
			if (n_neigh_same_lake<7) {
				splitting = Divide_Lake(row, col);
		...
		else if (row != 0 && row != Ny-1 && col == Nx-1){
			for (k=0; k<NDERS; k++) {
				if (IN_DOMAIN(ro2[k],co2[k])) {
					if (drainage[ro2[k]][co2[k]].lake == il) n_neigh_same_lake++;
				}
			}
			if (n_neigh_same_lake<7) {
				splitting = Divide_Lake(row, col);
...
int Delete_Node_From_Lake (int row, int col)
{
	...
	*/
	...
	int	imaxderneg=SIGNAL, ro[NDERS], co[NDERS], ro1[NDERS], co1[NDERS], ro2[NDERS], co2[NDERS], 
	ro3[NDERS-3], co3[NDERS-3], ro4[NDERS-3], co4[NDERS-3], ro5[NDERS-3], co5[NDERS-3], ro6[NDERS-3], co6[NDERS-3];
	int 	lrow, lcol;
	...
	for (k=0; k<NDERS; k++) {
		...
		if (pbc == 1){
			if (j == 0 && i != 0 && i != Ny-1){
				if (IN_DOMAIN(ro1[k],co1[k])) {
					deriv = (topo[ro1[k]][co1[k]]-topo[i][j])/dist;
					if ((deriv<0 && deriv<maxderneg) || (deriv==0 && deriv<=maxderneg && drainage[ro1[k]][co1[k]].lake))
					{imaxderneg=k; maxderneg=deriv;}
				}				
			}
			else if (j == Nx-1 && i != 0 && i != Ny-1){
				if (IN_DOMAIN(ro2[k],co2[k])) {
					deriv = (topo[ro2[k]][co2[k]]-topo[i][j])/dist;
					if ((deriv<0 && deriv<maxderneg) || (deriv==0 && deriv<=maxderneg && drainage[ro2[k]][co2[k]].lake))
					{imaxderneg=k; maxderneg=deriv;}
				}				
			}
			else{
	...
	if (maxderneg==0 && (drainage[row][col].type=='E' || ild)) {
		...
		if (imaxderneg != SIGNAL) {
			...
			if (pbc == 1){
				if (col == 0 && row != 0 && row != Ny-1){
					drainage[row][col].dr_row = ro1[imaxderneg];
					drainage[row][col].dr_col = co1[imaxderneg];
				}
				else if (col == Nx-1 && row != 0 && row != Ny-1){
					drainage[row][col].dr_row = ro2[imaxderneg];
					drainage[row][col].dr_col = co2[imaxderneg];
				}
				else{
					drainage[row][col].dr_row = ro[imaxderneg];
					drainage[row][col].dr_col = co[imaxderneg];
...
int Divide_Lake (int row, int col /*lake node to be removed*/)
{
	...
	*/
	...
	int 	ro[NDERS], co[NDERS], ro1[NDERS], co1[NDERS], ro2[NDERS], co2[NDERS], 
			ro3[NDERS-3], co3[NDERS-3], ro4[NDERS-3], co4[NDERS-3], 
			ro5[NDERS-3], co5[NDERS-3], ro6[NDERS-3], co6[NDERS-3];
	float 	maxderneg=0;
...
/*??, pero importa !!*/	else if (first_endorheic || i<nlakes) {
		...
		if (col == 0 && row != 0 && row != Ny-1){
			...
			float dist, deriv;
				...
				if (IN_DOMAIN(ro1[k],co1[k])) {
					deriv = (topo[ro1[k]][co1[k]]-topo[i][j])/dist;
					/*I include deriv==0 because it can happen that the removed node is in a plane and needs to drain somewhere in that plane.*/
					if (deriv<=0 && deriv<=maxderneg && drainage[ro1[k]][co1[k]].lake!=first_open) {
						imaxderneg=k;
						maxderneg=deriv;
					}
				}
			...
			float dist, deriv;
				...
				if (IN_DOMAIN(ro2[k],co2[k])) {
					deriv = (topo[ro2[k]][co2[k]]-topo[i][j])/dist;
					/*I include deriv==0 because it can happen that the removed node is in a plane and needs to drain somewhere in that plane.*/
					if (deriv<=0 && deriv<=maxderneg && drainage[ro2[k]][co2[k]].lake!=first_open) {
						imaxderneg=k;
						maxderneg=deriv;
					}
				}
	...
	if (imaxderneg != SIGNAL) {
		...
		if (pbc == 1){
			if (col == 0 && row != 0 && row != Ny-1){
				if (imaxderneg != SIGNAL) {
					new_dr_row = ro1[imaxderneg];
		 			new_dr_col = co1[imaxderneg];
				}
			}
			else if (col == Nx-1 && row != 0 && row != Ny-1){
				if (imaxderneg != SIGNAL) {
					new_dr_row = ro2[imaxderneg];
		 			new_dr_col = co2[imaxderneg];
				}
			}
			else{
				new_dr_row = ro[imaxderneg];
...
float Lake_Input_Discharge (int ilake)
{
	...
	*/
	...
	for (i=0; i<Lake[ilake].n_sd; i++) {
		...
		// D8 definition on the Western edge (not on vertices)
		ro1[0]=m-1, ro1[1]=m,   ro1[2]=m+1, ro1[3]=m,    ro1[4]=m-1, ro1[5]=m+1, ro1[6]=m+1,  ro1[7]=m-1;
		co1[0]=n,   co1[1]=n+1, co1[2]=n,   co1[3]=Nx-1,   co1[4]=n+1, co1[5]=n+1, co1[6]=Nx-1,   co1[7]=Nx-1;
		// D8 definition on the Eastern edge (not on vertices)
		ro2[0]=m-1, ro2[1]=m, ro2[2]=m+1, ro2[3]=m,   ro2[4]=m-1, ro2[5]=m+1, ro2[6]=m+1, ro2[7]=m-1;
		co2[0]=n,   co2[1]=0,   co2[2]=n,   co2[3]=n-1, co2[4]=0,     co2[5]=0,     co2[6]=n-1, co2[7]=n-1;
		// NW (0, 0)
		ro3[0]=m,    ro3[1]=m,   ro3[2]=m+1,  ro3[3]=m+1, ro3[4]=m+1;
		co3[0]=Nx-1, co3[1]=n+1, co3[2]=Nx-1, co3[3]=n,   co3[4]=n+1;
		// SW (Ny-1, 0)
		ro4[0]=m,    ro4[1]=m,   ro4[2]=m-1,  ro4[3]=m-1, ro4[4]=m-1;
		co4[0]=Nx-1, co4[1]=n+1, co4[2]=Nx-1, co4[3]=n,   co4[4]=n+1;
		// NE (0, Nx-1)
		ro5[0]=m,    ro5[1]=m,   ro5[2]=m+1,  ro5[3]=m+1, ro5[4]=m+1;
		co5[0]=n-1,  co5[1]=0,   co5[2]=n-1,  co5[3]=n,   co5[4]=0;
		// SE (Ny-1, Nx-1)
		ro6[0]=m,    ro6[1]=m,   ro6[2]=m-1,  ro6[3]=m-1, ro6[4]=m-1;
		co6[0]=n-1,  co6[1]=0,   co6[2]=n-1,  co6[3]=n,   co6[4]=0;
		...
		if (pbc == 1){
			for (k=0; k<NDERS; k++) {
				if (m != 0 && m != Ny-1 && n == 0){
					if (IN_DOMAIN(ro1[k],co1[k])) if (drainage[ro1[k]][co1[k]].dr_row == m && drainage[ro1[k]][co1[k]].dr_col == n && drainage[ro1[k]][co1[k]].lake != ilake)
					input_discharge += drainage[ro1[k]][co1[k]].discharge;
				}
				else if (m != 0 && m != Ny-1 && n == Nx-1){
					if (IN_DOMAIN(ro2[k],co2[k])) if (drainage[ro2[k]][co2[k]].dr_row == m && drainage[ro2[k]][co2[k]].dr_col == n && drainage[ro2[k]][co2[k]].lake != ilake)
					input_discharge += drainage[ro2[k]][co2[k]].discharge;
				}
				else{
					if (IN_DOMAIN(ro[k],co[k])) if (drainage[ro[k]][co[k]].dr_row == m && drainage[ro[k]][co[k]].dr_col == n && drainage[ro[k]][co[k]].lake != ilake)
					input_discharge += drainage[ro[k]][co[k]].discharge;
		...
		if (pbc == 2){
			if (Lake[ilake].row[i] == 0 && Lake[ilake].col[i] == 0){
				for (k=0; k<NDERS-3; k++) {
					if (IN_DOMAIN(ro3[k],co3[k])) if (drainage[ro3[k]][co3[k]].dr_row == m && drainage[ro3[k]][co3[k]].dr_col == n && drainage[ro3[k]][co3[k]].lake != ilake)
						input_discharge += drainage[ro3[k]][co3[k]].discharge;
				}
			}
			else if (Lake[ilake].row[i] == Ny-1 && Lake[ilake].col[i] == 0){
				for (k=0; k<NDERS-3; k++) {
					if (IN_DOMAIN(ro4[k],co4[k])) if (drainage[ro4[k]][co4[k]].dr_row == m && drainage[ro4[k]][co4[k]].dr_col == n && drainage[ro4[k]][co4[k]].lake != ilake)
						input_discharge += drainage[ro4[k]][co4[k]].discharge;
				}
			}
			else if (Lake[ilake].row[i] == 0 && Lake[ilake].col[i] == Nx-1){
				for (k=0; k<NDERS-3; k++) {
					if (IN_DOMAIN(ro5[k],co5[k])) if (drainage[ro5[k]][co5[k]].dr_row == m && drainage[ro5[k]][co5[k]].dr_col == n && drainage[ro5[k]][co5[k]].lake != ilake)
						input_discharge += drainage[ro5[k]][co5[k]].discharge;
				}
			}
			else if (Lake[ilake].row[i] == Ny-1 && Lake[ilake].col[i] == Nx-1){
				for (k=0; k<NDERS-3; k++) {
					if (IN_DOMAIN(ro6[k],co6[k])) if (drainage[ro6[k]][co6[k]].dr_row == m && drainage[ro6[k]][co6[k]].dr_col == n && drainage[ro6[k]][co6[k]].lake != ilake)
						input_discharge += drainage[ro6[k]][co6[k]].discharge;
				}
			}
			else if (Lake[ilake].row[i] != 0 && Lake[ilake].row[i] != Ny-1 && Lake[ilake].col[i] == 0){
				for (k=0; k<NDERS; k++) {
					if (IN_DOMAIN(ro1[k],co1[k])) if (drainage[ro1[k]][co1[k]].dr_row == m && drainage[ro1[k]][co1[k]].dr_col == n && drainage[ro1[k]][co1[k]].lake != ilake)
						input_discharge += drainage[ro1[k]][co1[k]].discharge;
				}
			}
			else if (Lake[ilake].row[i] != 0 && Lake[ilake].row[i] != Ny-1 && Lake[ilake].col[i] == Nx-1){
				for (k=0; k<NDERS; k++) {
					if (IN_DOMAIN(ro2[k],co2[k])) if (drainage[ro2[k]][co2[k]].dr_row == m && drainage[ro2[k]][co2[k]].dr_col == n && drainage[ro2[k]][co2[k]].lake != ilake)
						input_discharge += drainage[ro2[k]][co2[k]].discharge;
				}
...
float Minimum_Neg_Slope (int i, int j, int *dr_row, int *dr_col)
{
	...
	*/
	int 	k, ro[NDERS], co[NDERS], ro1[NDERS], co1[NDERS], ro2[NDERS], co2[NDERS], iminderneg=SIGNAL;
	int ro3[NDERS-3], co3[NDERS-3];
	int ro4[NDERS-3], co4[NDERS-3];
	int ro5[NDERS-3], co5[NDERS-3];
	int ro6[NDERS-3], co6[NDERS-3];
	...
	if (iminderneg != SIGNAL) {
		...
		if (pbc == 1){
			if (j == 0 && i != 0 && i != Ny-1){
				*dr_row = ro1[iminderneg];
				*dr_col = co1[iminderneg];
			}
			else if (j == Nx-1 && i != 0 && i != Ny-1){
				*dr_row = ro2[iminderneg];
				*dr_col = co2[iminderneg];
			}
			else{
				*dr_row = ro[iminderneg];
				*dr_col = co[iminderneg];
...
int Orographic_Precipitation_Evaporation_conservative (float windvel, float windazimut, float relative_humidity)
{
	/*
	  ...
	  Based on Roe et al., 2003, JGR, modified by Garcia-Castellanos, 2007, EPSL. See also Masek et al., 1994, JGR.
	  It calculates the max. water content in every column as a function of temperature. Evaporation depends on wind velocity
	*/
	int 	i, j, ib, jb, istart=0, iincr=0, jstart=0, jincr=0;
	float 	Wcol, Wmax, 	/*Water content in a column, and maximum water content*/
		z, dz=50, dtwind;
	float 	windvelx, windvely; /*[m/s]*/
	float 	**done;
...
int write_topo(icount1)
{
	int 	i, j;
	FILE 	*file;

	char name[MAXLENFILE]; 
	sprintf(name, "topo_fine_%2.2f_%03d.xyzf", Time/Matosec, icount1); 
	//remove(name); 
	if ((file=fopen(name,"wt"))==NULL) {
		PRINT_WARNING("Could not open output file '%s'.",name);
		return 0;
	}
	...
	fclose(file);
		//char command2[300];
		//sprintf(command2, "cp topo_fine.xyzf %s%04d_%04d.xyzf", "fan", Time/Matosec, n_iters);
		//system(command2);
		//$$
	return (1);
}
